...
const { verifyUserCredentials, addUserLogin } = require('./Login.service');
const { getHotelId, getUserName, isAuthenticatedUser } = require('../../common/services/common.service.js');
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!isValidUser) {
            ...
            console.info(`${username} -> FAILED Login attempt for user ${username} with hotelId ${hotelId} `)
            return;
        }
...
router.post('/addUser', async (req, res) => {
    ...
    console.info(`${exisitngUserName} -> Attempting to add new user: ${username} for hotelId ${hotelId}`);

    // Validate input
    if (!username || !password || !hotelId) {
        console.error(`${exisitngUserName} -> Username: ${username}, hotelId ${hotelId}, password: ${password} :: all fields not present`);
        res.status(400).json({ message: "Username, hotelId, and password are required" });
        return;
    }
