...
# Skip venv setup — assume Conda is already active
# VENV=${VENV-$(pwd)/.venv}
...
if [ -z "$CONDA_DEFAULT_ENV" ]; then
    echo "Please activate your Conda env (e.g., conda activate precise-env) before running this script"
    exit 1
fi
...
# Replace with a Conda environment check
if [ -z "$CONDA_DEFAULT_ENV" ]; then
    echo "❌ Please activate your Conda environment first!"
    echo "➡️  Run: conda activate precise-env"
    exit 1
fi
