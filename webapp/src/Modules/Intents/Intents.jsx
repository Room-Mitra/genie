...
const Intents = () => {
        ...
        if (speech?.role) {
          if (speech.role.toLowerCase() === 'user') {
            ...
            try {
              ...
              console.error('Error parsing assistant message');
            }
          }
        }
      });
  ...
  const [colDefs] = useState([
    {
      headerName: 'Date',
      valueGetter: (p) => new Date(p.data.requestedTime).toLocaleDateString(),
      filter: true,
      width: 110,
    },
    {
      headerName: 'Time',
      valueGetter: (p) => new Date(p.data.requestedTime).toLocaleTimeString(),
      filter: true,
      width: 110,
    },
    ...
    { headerName: 'Room', field: 'roomId', filter: true, editable: false, width: 210 },
    {
      ...
      field: 'acknowledged',
      cellRenderer: AcknowledgeCheckboxComponent,
      width: 130,
    },
      ...
      <tbody>
        {data.map((row, index) => (
          <tr key={index}>
            ...
            <td style={{ width: '80%', padding: '8px', borderBottom: '1px solid #ddd' }}>
              {row.value}
            </td>
          </tr>
        ))}
