...
const StaffDirectory = () => {
  ...
  useEffect(() => {
    const fetchStaffList = async () => {
      ...
      staffData.forEach((staff) => {
        staff.department = staff.department.map((department) => ({
          value: department,
          label: department,
        }));
  ...
  const handleAddStaff = (e) => {
    ...
    if (
      !currentStaff.name ||
      !currentStaff.phone ||
      !currentStaff.department.length ||
      !currentStaff.role.length
    ) {
  ...
  return (
    <div style={ComponentContainerStyle}>
      ...
      {showDeleteConfirmation && (
        <div className="popup">
          <p>
            Are you sure you want to delete this staff member?{`(${staffList[staffToDelete].name})`}
          </p>
          <BlueGoldButton clickHandler={confirmDeleteStaff} text={'Yes'} />
          <SecondaryButton clickHandler={cancelDeleteStaff} text={'No'} />
        </div>
      )}
      ...
      >
        ...
        <tbody>
          {filteredStaffList.map((staff, index) => (
            <tr key={index}>
              <td key={index} style={TableDataStyle}>
                {staff['name']}
              </td>
              <td key={index} style={TableDataStyle}>
                {staff['phone']}
              ...
              <td key={index} style={TableDataStyle}>
                {staff['department'].map((x) => x.value).join(', ')}
              </td>
              <td key={index} style={TableDataStyle}>
                {staff['role'].map((x) => x.value).join(', ')}
              </td>
